			...
			if shouldSelectChannel {
				...
				if channel == nil {
					...
					if err != nil {
						...
						logger.LogWarn(c, fmt.Sprintf("get channel failed for group %s model %s (distributor): %s", showGroup, modelRequest.Model, err.Error()))
						abortWithOpenAiMessage(c, http.StatusServiceUnavailable, "The model is currently unavailable, please try again later", types.ErrorCodeModelNotFound)
						return
					}
					if channel == nil {
						logger.LogWarn(c, fmt.Sprintf("no available channel for group %s model %s (distributor)", usingGroup, modelRequest.Model))
						abortWithOpenAiMessage(c, http.StatusServiceUnavailable, "The model is currently unavailable, please try again later", types.ErrorCodeModelNotFound)
						return
					}
				}
